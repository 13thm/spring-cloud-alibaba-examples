 ...
 * <ol>
 *   <li>从 Nacos 拉取 integrated-account 的实例列表并负载均衡选址；</li>
 *   <li>把方法参数序列化为 JSON 并发起 HTTP POST；</li>
 *   <li>把响应反序列化为 Result 返回；</li>
 *   <li>（配合 Seata）在请求头中透传全局事务 XID —— 这是跨服务事务传播的关键。</li>
 * </ol>
 *
 * <p>注意：方法路径/参数必须与 AccountController 的接口完全一致，
 * 否则 404。接口名 name="integrated-account" 即 Nacos 注册的服务名。</p>
 *
 * @author TrevorLink
 */
