 ...
 * BusinessException</b>，让 AOP 感知失败触发回滚 —— 这是本示例最容易被忽略的细节。</p>
 *
 * @author TrevorLink
 */
@Service
public class OrderServiceImpl implements OrderService {
	...
	/** 账户服务 Feign 客户端（发 HTTP 调用 integrated-account） */
	@Autowired
	private AccountServiceFeignClient accountService;

	/** 库存服务 Feign 客户端（发 HTTP 调用 integrated-storage） */
	@Autowired
	...
	public Result<?> createOrder(String userId, String commodityCode, Integer count) {
		...
		// ---------- 第 1 步：扣减库存（Feign → integrated-storage） ----------
		...
		// ---------- 第 2 步：扣减账户余额（Feign → integrated-account） ----------
