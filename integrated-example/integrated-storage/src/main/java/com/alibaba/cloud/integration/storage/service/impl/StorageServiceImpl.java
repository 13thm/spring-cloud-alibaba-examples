 ...
 * <p>关键点逐条解析：</p>
 * <ol>
 *   <li>方法上的 {@code @Transactional} 是普通的 Spring 本地事务注解；
 *       Seata 启动时自动代理了 DataSource，本地事务提交/回滚会同步
 *       向 TC（Seata Server）注册分支并上报状态，开发者无感知。</li>
 *   <li>{@link RootContext#getXID()}：获取全局事务 ID。日志里打印 XID
 *       可以验证"Feign 调用链上 XID 成功传播"（XID 通过请求头传递），
 *       这是 AT 模式能把多个服务的本地事务串成全局事务的基石。</li>
 *   <li>业务失败抛 {@link BusinessException}：最终会让订单服务（全局事务发起方）
 *       感知失败并触发全局回滚。</li>
 * </ol>
 *
 * @author TrevorLink
...
public class StorageServiceImpl implements StorageService {
	 ...
	 * 余额/库存不足抛异常，是本示例触发 Seata 全局回滚的"人为埋点"。
	 */
